...
#include "system_wrappers/interface/bitbuffer.h"
...
size_t RefIndicesLength(const RTPVideoHeaderVP9& hdr) {
  ...
  CHECK_GT(hdr.num_ref_pics, 0U);
  CHECK_LE(hdr.num_ref_pics, kMaxVp9RefPics);
...
size_t SsDataLength(const RTPVideoHeaderVP9& hdr) {
  ...
  CHECK_GT(hdr.num_spatial_layers, 0U);
  CHECK_LE(hdr.num_spatial_layers, kMaxVp9NumberOfSpatialLayers);
  CHECK_LE(hdr.gof.num_frames_in_gof, kMaxVp9FramesInGof);
  size_t length = 1;                           // V
  ...
  for (size_t i = 0; i < hdr.gof.num_frames_in_gof; ++i) {
    CHECK_LE(hdr.gof.num_ref_pics[i], kMaxVp9RefPics);
...
bool WriteSsData(const RTPVideoHeaderVP9& vp9, BitBufferWriter* writer) {
  CHECK_GT(vp9.num_spatial_layers, 0U);
  CHECK_LE(vp9.num_spatial_layers, kMaxVp9NumberOfSpatialLayers);
  CHECK_LE(vp9.gof.num_frames_in_gof, kMaxVp9FramesInGof);
...
//      +-+-+-+-+-+-+-+-+
//
bool ParsePictureId(BitBuffer* parser, RTPVideoHeaderVP9* vp9) {
  uint32_t picture_id;
  uint32_t m_bit;
...
//      +-+-+-+-+-+-+-+-+
//
bool ParseLayerInfoCommon(BitBuffer* parser, RTPVideoHeaderVP9* vp9) {
...
//      +-+-+-+-+-+-+-+-+
//
bool ParseLayerInfoNonFlexibleMode(BitBuffer* parser,
...
bool ParseLayerInfo(BitBuffer* parser, RTPVideoHeaderVP9* vp9) {
...
//                                                current P_DIFF.
//
bool ParseRefIndices(BitBuffer* parser, RTPVideoHeaderVP9* vp9) {
...
//      +-+-+-+-+-+-+-+-+              -|           -|
//
bool ParseSsData(BitBuffer* parser, RTPVideoHeaderVP9* vp9) {
...
bool RtpPacketizerVp9::WriteHeader(const PacketInfo& packet_info,
  ...
  BitBufferWriter writer(buffer, max_payload_length_);
...
bool RtpDepacketizerVp9::Parse(ParsedPayload* parsed_payload,
  ...
  BitBuffer parser(payload, payload_length);
