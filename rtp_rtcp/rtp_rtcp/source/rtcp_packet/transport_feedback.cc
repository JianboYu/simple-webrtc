...
void TransportFeedback::WithBase(uint16_t base_sequence,
  ...
  CHECK_EQ(-1, base_seq_);
  CHECK_NE(-1, ref_timestamp_us);
...
void TransportFeedback::EmitRunLengthChunk() {
  CHECK_GE(first_symbol_cardinality_, symbol_vec_.size());
...
std::unique_ptr<TransportFeedback> TransportFeedback::ParseFrom(
  ...
  CHECK_EQ(num_packets, symbols.size());
  ...
  CHECK_GE(index, end_index - 3);
  CHECK_LE(index, end_index);
