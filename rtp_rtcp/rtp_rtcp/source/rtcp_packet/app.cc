...
bool App::Parse(const RtcpCommonHeader& header, const uint8_t* payload) {
  CHECK(header.packet_type == kPacketType);
...
void App::WithSubType(uint8_t subtype) {
  CHECK_LE(subtype, 0x1f);
  sub_type_ = subtype;
}

void App::WithData(const uint8_t* data, size_t data_length) {
  CHECK(data);
  CHECK_EQ(0u, data_length % 4) << "Data must be 32 bits aligned.";
  CHECK(data_length <= kMaxDataSize) << "App data size << " << data_length
                                          << "exceed maximum of "
                                          << kMaxDataSize << " bytes.";
