...
#include "rtp_rtcp/source/rtp_packet.h"

#include <cstring>

#include "system_wrappers/interface/checks.h"
#include "system_wrappers/interface/logging.h"
#include "system_wrappers/interface/random.h"
#include "system_wrappers/interface/common_types.h"
#include "rtp_rtcp/include/rtp_header_extension.h"
#include "rtp_rtcp/source/rtp_header_extensions.h"
...
Packet::Packet(const ExtensionManager* extensions, size_t capacity)
  ...
  CHECK_GE(capacity, kFixedHeaderSize);
  Clear();
}
...
bool Packet::Parse(const uint8_t* buffer, size_t buffer_size) {
  ...
  CHECK_EQ(size(), buffer_size);
  buffer_.SetData(buffer, buffer_size);
  return true;
}

bool Packet::Parse(Buffer buffer) {
...
bool Packet::Marker() const {
  CHECK_EQ(marker_, (data()[1] & 0x80) != 0);
  return marker_;
}

uint8_t Packet::PayloadType() const {
  CHECK_EQ(payload_type_, data()[1] & 0x7f);
  return payload_type_;
}

uint16_t Packet::SequenceNumber() const {
  CHECK_EQ(sequence_number_,
                ByteReader<uint16_t>::ReadBigEndian(data() + 2));
  return sequence_number_;
}

uint32_t Packet::Timestamp() const {
  CHECK_EQ(timestamp_, ByteReader<uint32_t>::ReadBigEndian(data() + 4));
  return timestamp_;
}

uint32_t Packet::Ssrc() const {
  CHECK_EQ(ssrc_, ByteReader<uint32_t>::ReadBigEndian(data() + 8));
  return ssrc_;
}

std::vector<uint32_t> Packet::Csrcs() const {
  size_t num_csrc = data()[0] & 0x0F;
  CHECK_GE(capacity(), kFixedHeaderSize + num_csrc * 4);
...
void Packet::CopyHeader(const Packet& packet) {
  CHECK_GE(capacity(), packet.headers_size());
...
void Packet::SetCsrcs(const std::vector<uint32_t>& csrcs) {
  CHECK_EQ(num_extensions_, 0u);
  CHECK_EQ(payload_size_, 0u);
  CHECK_EQ(padding_size_, 0u);
  CHECK_LE(csrcs.size(), 0x0fu);
  CHECK_LE(kFixedHeaderSize + 4 * csrcs.size(), capacity());
...
void Packet::SetPayloadSize(size_t size_bytes) {
  CHECK_EQ(padding_size_, 0u);
  CHECK_LE(size_bytes, payload_size_);
  payload_size_ = size_bytes;
}
...
bool Packet::AllocateExtension(ExtensionType type,
  ...
  if (num_extensions_ == 0) {
    CHECK_EQ(payload_offset_, kFixedHeaderSize + (num_csrc * 4));
    CHECK_EQ(extensions_size_, 0);
  ...
  CHECK(num_extensions_ < kMaxExtensionHeaders);
