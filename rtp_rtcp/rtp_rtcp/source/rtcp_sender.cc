...
#include "rtp_rtcp/source/rtcp_sender.h"
...
#include "rtp_rtcp/include/rtc_event_log.h"
#include "system_wrappers/interface/common_types.h"
#include "rtp_rtcp/source/rtcp_packet/app.h"
#include "rtp_rtcp/source/rtcp_packet/bye.h"
#include "rtp_rtcp/source/rtcp_packet/compound_packet.h"
#include "rtp_rtcp/source/rtcp_packet/extended_reports.h"
#include "rtp_rtcp/source/rtcp_packet/fir.h"
#include "rtp_rtcp/source/rtcp_packet/nack.h"
#include "rtp_rtcp/source/rtcp_packet/pli.h"
#include "rtp_rtcp/source/rtcp_packet/receiver_report.h"
#include "rtp_rtcp/source/rtcp_packet/remb.h"
#include "rtp_rtcp/source/rtcp_packet/rpsi.h"
#include "rtp_rtcp/source/rtcp_packet/sdes.h"
#include "rtp_rtcp/source/rtcp_packet/sender_report.h"
#include "rtp_rtcp/source/rtcp_packet/sli.h"
#include "rtp_rtcp/source/rtcp_packet/tmmbn.h"
#include "rtp_rtcp/source/rtcp_packet/tmmbr.h"
#include "rtp_rtcp/source/rtcp_packet/transport_feedback.h"
#include "rtp_rtcp/source/rtp_rtcp_impl.h"
#include "rtp_rtcp/source/tmmbr_help.h"
...
class PacketContainer : public rtcp::CompoundPacket,
 ...
 public:
  ...
  size_t SendPackets(size_t max_payload_length) {
    CHECK_LE(max_payload_length, static_cast<size_t>(IP_PACKET_SIZE));
 ...
 private:
  ...
  DISALLOW_IMPLICIT_CONSTRUCTORS(PacketContainer);
};
...
RTCPSender::RTCPSender(
  ...
  critical_section_rtcp_sender_.reset(CriticalSectionWrapper::CreateCriticalSection());
}

RTCPSender::~RTCPSender() {}

RtcpMode RTCPSender::Status() const {
  CriticalSectionScoped cs(critical_section_rtcp_sender_.get());
  return method_;
}
...
int32_t RTCPSender::SetCNAME(const char* c_name) {
  ...
  CHECK_LT(strlen(c_name), static_cast<size_t>(RTCP_CNAME_SIZE));
  CriticalSectionScoped cs(critical_section_rtcp_sender_.get());
  cname_ = c_name;
  return 0;
}
...
int32_t RTCPSender::SendCompoundRTCP(
  ...
  PacketContainer container(transport_, event_log_);
  {
    ...
    while (it != report_flags_.end()) {
      ...
      CHECK(builder_it != builders_.end());
      if (it->is_volatile) {
    ...
    CHECK(AllVolatileFlagsConsumed());
  }
...
void RTCPSender::PrepareReport(const std::set<RTCPPacketType>& packetTypes,
  ...
  if (IsFlagPresent(kRtcpSr) || IsFlagPresent(kRtcpRr)) {
    ...
    CHECK(ConsumeFlag(kRtcpReport) == false);
  } else {
    generate_report =
...
bool RTCPSender::AddReportBlock(const FeedbackState& feedback_state,
  ...
  CHECK(report_blocks_.find(ssrc) == report_blocks_.end());
...
void RTCPSender::SetCsrcs(const std::vector<uint32_t>& csrcs) {
  CHECK_LE(csrcs.size(), static_cast<size_t>(kRtpCsrcSize));
  CriticalSectionScoped cs(critical_section_rtcp_sender_.get());
  csrcs_ = csrcs;
}
...
bool RTCPSender::SendFeedbackPacket(const rtcp::TransportFeedback& packet) {
  ...
  CHECK_LE(max_payload_length_, static_cast<size_t>(IP_PACKET_SIZE));
  uint8_t buffer[IP_PACKET_SIZE];
