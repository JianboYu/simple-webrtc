...
#include "rtp_rtcp/source/forward_error_correction_internal.h"
...
int ForwardErrorCorrection::GenerateFec(const PacketList& media_packet_list,
  ...
  CHECK_GT(num_media_packets, 0);
  CHECK_GE(num_important_packets, 0);
  CHECK_LE(num_important_packets, num_media_packets);
  CHECK(fec_packet_list->empty());
...
int ForwardErrorCorrection::GetNumberOfFecPackets(int num_media_packets,
  ...
  CHECK_LE(num_fec_packets, num_media_packets);
...
void ForwardErrorCorrection::ResetState(
  ...
  CHECK(recovered_packet_list->empty());
  ...
  while (!fec_packet_list_.empty()) {
    ...
    CHECK(fec_packet->protected_pkt_list.empty());
    delete fec_packet;
    fec_packet_list_.pop_front();
  }
  CHECK(fec_packet_list_.empty());
}
...
void ForwardErrorCorrection::InsertFecPacket(
  ...
  if (fec_packet->protected_pkt_list.empty()) {
    ...
    CHECK_LE(fec_packet_list_.size(), kMaxFecPackets);
  }
}
...
void ForwardErrorCorrection::InsertPackets(
  ...
  CHECK(received_packet_list->empty());
...
void ForwardErrorCorrection::DiscardOldPackets(
  ...
  CHECK(recovered_packet_list->size() <= kMaxMediaPackets);
}
