...
#include "system_wrappers/interface/config.h"
...
RtpRtcp::Configuration::Configuration()
    : audio(false),
      ...
      transport_feedback_callback(NULL),
      rtt_stats(NULL),
      rtcp_packet_type_counter_observer(NULL),
      remote_bitrate_estimator(NULL),
      paced_sender(NULL),
      transport_sequence_number_allocator(NULL),
      send_bitrate_observer(NULL),
      send_frame_count_observer(NULL),
      send_side_delay_observer(NULL),
      event_log(NULL),
      send_packet_observer(NULL),
      retransmission_rate_limiter(NULL) {}
...
ModuleRtpRtcpImpl::ModuleRtpRtcpImpl(const Configuration& configuration)
  ...
  critical_section_rtt_.reset(CriticalSectionWrapper::CreateCriticalSection());
}
...
int32_t ModuleRtpRtcpImpl::SetMaxTransferUnit(uint16_t mtu) {
  CHECK_LE(mtu, IP_PACKET_SIZE) << "MTU too large: " << mtu;
  CHECK_GT(mtu, packet_overhead_) << "MTU too small: " << mtu;
