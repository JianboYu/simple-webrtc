...
#include "udp_transport/source/udp_socket_posix.h"
...
#include "udp_transport/source/udp_socket_manager_wrapper.h"
#include "udp_transport/source/udp_socket_wrapper.h"
    ...
    : _id(id)
{
    ...
    _closeBlockingCompletedCond.reset(EventWrapper::Create());
    _readyForDeletionCond.reset(EventWrapper::Create());
    _cs.reset(CriticalSectionWrapper::CreateCriticalSection());
}
...
{
    CriticalSectionScoped lock(_cs.get());
    ...
    {
        _cs->Leave();
        _readyForDeletionCond->Wait(WEBRTC_EVENT_INFINITE);
        _cs->Enter();
    }
...
void UdpSocketPosix::ReadyForDeletion()
{
    ...
    {
        _cs->Leave();
        _closeBlockingCompletedCond->Wait(WEBRTC_EVENT_INFINITE);
        _cs->Enter();
    }
}
