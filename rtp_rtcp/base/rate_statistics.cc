...
void RateStatistics::Update(size_t count, int64_t now_ms) {
  ...
  CHECK_LT(now_offset, max_window_size_ms_);
...
Optional<uint32_t> RateStatistics::Rate(int64_t now_ms) const {
...
void RateStatistics::EraseOld(int64_t now_ms) {
  ...
  while (num_samples_ > 0 && oldest_time_ < new_oldest_time) {
    ...
    CHECK_GE(accumulated_count_, oldest_bucket.sum);
    CHECK_GE(num_samples_, oldest_bucket.samples);
