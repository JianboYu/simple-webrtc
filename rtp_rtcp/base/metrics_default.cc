...
#include "base/metrics_default.h"
...
class RtcHistogram {
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(RtcHistogram);
};
...
void CreateMap() {
  RtcHistogramMap* map = AtomicOps::AcquireLoadPtr(&g_rtc_histogram_map);
  if (map == nullptr) {
    RtcHistogramMap* new_map = new RtcHistogramMap();
    RtcHistogramMap* old_map = AtomicOps::CompareAndSwapPtr(
...
#if RTC_DCHECK_IS_ON
  AtomicOps::ReleaseStore(&g_rtc_histogram_called, 1);
#endif
...
void HistogramAdd(Histogram* histogram_pointer,
  ...
  CHECK_EQ(name, ptr->name()) << "The name should not vary.";
  ptr->Add(sample);
}
...
void Enable() {
  CHECK(g_rtc_histogram_map == nullptr);
#if RTC_DCHECK_IS_ON
  CHECK_EQ(0, AtomicOps::AcquireLoad(&g_rtc_histogram_called));
#endif
  CreateMap();
}
