...
#include "system_wrappers/interface/buffer.h"
#include "system_wrappers/interface/logging.h"

#define RETURN_EMPTY_ON_FAIL(x)                  \
  if (!(x)) {                                    \
    return Optional<PpsParser::PpsState>(); \
  }
...
Optional<PpsParser::PpsState> PpsParser::ParsePps(const uint8_t* data,
                                                       size_t length) {
...
Optional<PpsParser::PpsState> PpsParser::ParseInternal(
  ...
  return Optional<PpsParser::PpsState>(pps);
}
