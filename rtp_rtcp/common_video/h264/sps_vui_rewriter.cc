...
bool CopyHrdParameters(BitBuffer* source,
                       BitBufferWriter* destination);
bool AddBitstreamRestriction(BitBufferWriter* destination,
                             uint32_t max_num_ref_frames);
bool CopyRemainingBits(BitBuffer* source,
                       BitBufferWriter* destination);
...
SpsVuiRewriter::ParseResult SpsVuiRewriter::ParseAndRewriteSps(
    ...
    Buffer* destination) {
  BitBuffer source_buffer(buffer, length);
  Optional<SpsParser::SpsState> sps_state =
  ...
  *sps = sps_state;
  ...
  // write with.
  Buffer out_buffer(length + kMaxVuiSpsIncrease);
  ...
  CHECK(byte_offset <= length + kMaxVuiSpsIncrease);
