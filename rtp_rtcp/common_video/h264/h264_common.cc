...
std::vector<NaluIndex> FindNaluIndices(const uint8_t* buffer,
  ...
  CHECK_GE(buffer_size, kNaluShortStartSequenceSize);
...
std::unique_ptr<Buffer> ParseRbsp(const uint8_t* data, size_t length) {
  std::unique_ptr<Buffer> rbsp_buffer(new Buffer(0, length));
...
void WriteRbsp(const uint8_t* bytes, size_t length, Buffer* destination) {
