...
#include "system_wrappers/interface/bytebuffer.h"
#include "system_wrappers/interface/logging.h"
#include "rtp_rtcp/include/optional.h"

typedef webrtc::Optional<webrtc::SpsParser::SpsState> OptionalSps;
...
Optional<SpsParser::SpsState> SpsParser::ParseSps(const uint8_t* data,
                                                       size_t length) {
  std::unique_ptr<Buffer> unpacked_buffer = H264::ParseRbsp(data, length);
  BitBuffer bit_buffer(unpacked_buffer->data(), unpacked_buffer->size());
  return ParseSpsUpToVui(&bit_buffer);
}

Optional<SpsParser::SpsState> SpsParser::ParseSpsUpToVui(
    BitBuffer* buffer) {
