...
#define LAZY_STREAM(stream, condition)                                        \
  !(condition) ? static_cast<void>(0) : webrtc::FatalMessageVoidify() & (stream)

// The actual stream used isn't important.
#define EAT_STREAM_PARAMETERS                                           \
  true ? static_cast<void>(0)                                           \
       : webrtc::FatalMessageVoidify() & webrtc::FatalMessage("", 0).stream()
...
#define CHECK(condition)                                                    \
  LAZY_STREAM(webrtc::FatalMessage(__FILE__, __LINE__).stream(), !(condition)) \
...
#define CHECK_OP(name, op, val1, val2)                      \
  if (std::string* _result =                                \
      webrtc::Check##name##Impl((val1), (val2),                \
                             #val1 " " #op " " #val2))      \
    webrtc::FatalMessage(__FILE__, __LINE__, _result).stream()
...
#define DEFINE_CHECK_OP_IMPL(name, op) \
  ...
  inline std::string* Check##name##Impl(const t1& v1, const t2& v2, \
    ...
    else return webrtc::MakeCheckOpString(v1, v2, names); \
  } \
...
#define FATAL() webrtc::FatalMessage(__FILE__, __LINE__).stream()
