...
uint8_t WritePartialByte(uint8_t source,
  ...
  CHECK(target_bit_offset < 8);
  CHECK(source_bit_count < 9);
  CHECK(source_bit_count <= (8 - target_bit_offset));
...
BitBuffer::BitBuffer(const uint8_t* bytes, size_t byte_count)
  ...
  CHECK(static_cast<uint64_t>(byte_count_) <=
...
bool BitBuffer::ReadUInt8(uint8_t* val) {
  ...
  CHECK(bit_val <= std::numeric_limits<uint8_t>::max());
...
bool BitBuffer::ReadUInt16(uint16_t* val) {
  ...
  CHECK(bit_val <= std::numeric_limits<uint16_t>::max());
...
bool BitBuffer::ReadExponentialGolomb(uint32_t* val) {
  ...
  CHECK(!PeekBits(&peeked_bit, 1) || peeked_bit == 1);
