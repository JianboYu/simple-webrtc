...
void Expand::Reset() {
  ...
  for (size_t ix = 0; ix < num_channels_; ++ix) {
    (channel_parameters_.get() + ix)->expand_vector0.Clear();
    (channel_parameters_.get() + ix)->expand_vector1.Clear();
  }
}
...
//      expand_lags_[2] = fs_mult * 120;
    ...
    GenerateBackgroundNoise(random_vector,
                            channel_ix,
                            (channel_parameters_.get() + channel_ix)->mute_slope,
                            TooManyExpands(),
                            current_lag,
...
void Expand::InitializeForAnExpandPeriod() {
  ...
  consecutive_expands_ = 0;
  for (size_t ix = 0; ix < num_channels_; ++ix) {
    (channel_parameters_.get() + ix)->current_voice_mix_factor = 16384;  // 1.0 in Q14.
    (channel_parameters_.get() + ix)->mute_factor = 16384;  // 1.0 in Q14.
