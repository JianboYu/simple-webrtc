...
#include "neteq/comfort_noise.h"
#include "neteq/decision_logic.h"
#include "neteq/decoder_database.h"
#include "neteq/defines.h"
#include "neteq/delay_manager.h"
#include "neteq/delay_peak_detector.h"
#include "neteq/dtmf_buffer.h"
#include "neteq/dtmf_tone_generator.h"
...
int NetEqImpl::Decode(PacketList* packet_list, Operations* operation,
  ...
  *decoded_length = 0;
  ...
  if ((*operation == kMerge) && decoder && decoder->HasDecodePlc()) {
    decoder->DecodePlc(1, decoded_buffer_.get() + *decoded_length);
  }
...
int NetEqImpl::DecodeLoop(PacketList* packet_list, Operations* operation,
  ...
  while (packet &&
    ...
    if (packet->sync_packet) {
      ...
      memset(decoded_buffer_.get() + *decoded_length, 0, decoder_frame_length_ *
             decoder->channels() * sizeof(decoded_buffer_.get()));
      ...
      decode_length = decoder->DecodeRedundant(
          ...
          decoded_buffer_.get() + *decoded_length, speech_type);
    } else {
      ...
      decode_length = decoder->Decode(packet->payload,
                                      ...
                                      decoded_buffer_.get() + *decoded_length,
                                      speech_type);
    }
...
void NetEqImpl::UpdatePlcComponents(int fs_hz, size_t channels) {
  ...
  Expand *pex = expand_factory_->Create(background_noise_.get(),
...
void NetEqImpl::SetSampleRateAndChannels(int fs_hz, size_t channels) {
  ...
  for (size_t i = 0; i < channels; ++i) {
    *(mute_factor_array_.get() + i) = 16384;  // 1.0 in Q14.
  }
