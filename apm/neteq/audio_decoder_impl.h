...
#include "common_audio/engine_configurations.h"
#endif
...
class AudioDecoderADPCM : public AudioDecoder {
 public:
  explicit AudioDecoderADPCM(enum NetEqDecoder type);
  virtual ~AudioDecoderADPCM();
  virtual int Decode(const uint8_t* encoded, size_t encoded_len,
                     int16_t* decoded, SpeechType* speech_type);
  virtual int Init();
  virtual int PacketDuration(const uint8_t* encoded, size_t encoded_len);
 private:
  DISALLOW_COPY_AND_ASSIGN(AudioDecoderADPCM);
};
#endif
#ifdef WEBRTC_CODEC_G722_1C
class AudioDecoderG722_1C : public AudioDecoder {
 public:
  explicit AudioDecoderG722_1C(enum NetEqDecoder type);
  virtual ~AudioDecoderG722_1C();
  virtual int Decode(const uint8_t* encoded, size_t encoded_len,
                     int16_t* decoded, SpeechType* speech_type);
  virtual int Init();
  virtual int PacketDuration(const uint8_t* encoded, size_t encoded_len);
 private:
  DISALLOW_COPY_AND_ASSIGN(AudioDecoderG722_1C);
};
#endif
...
class AudioDecoderAMR : public AudioDecoder {
 public:
  explicit AudioDecoderAMR(enum NetEqDecoder type);
...
class AudioDecoderAMRWB : public AudioDecoder {
 public:
  explicit AudioDecoderAMRWB(enum NetEqDecoder type);
  virtual ~AudioDecoderAMRWB();
  virtual int Decode(const uint8_t* encoded, size_t encoded_len,
                     int16_t* decoded, SpeechType* speech_type);
  virtual int Init();
  virtual int PacketDuration(const uint8_t* encoded, size_t encoded_len);
 private:
  DISALLOW_COPY_AND_ASSIGN(AudioDecoderAMRWB);
};
#endif

#ifdef WEBRTC_CODEC_AACLC
class AudioDecoderAACLC : public AudioDecoder {
 public:
  explicit AudioDecoderAACLC(enum NetEqDecoder type);
  virtual ~AudioDecoderAACLC();
  virtual int Decode(const uint8_t* encoded, size_t encoded_len,
                     int16_t* decoded, SpeechType* speech_type);
  virtual int Init();
  virtual int PacketDuration(const uint8_t* encoded, size_t encoded_len);
 private:
  DISALLOW_COPY_AND_ASSIGN(AudioDecoderAACLC);
};
#if 0
class AudioDecoderAACLCMultiCh : public AudioDecoderAACLC {
 public:
  explicit AudioDecoderAACLCMultiCh(enum NetEqDecoder type);

 private:
  DISALLOW_COPY_AND_ASSIGN(AudioDecoderAACLCMultiCh);
};
#endif
#endif
