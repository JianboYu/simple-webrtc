...
void AudioVector::PushFront(const AudioVector& prepend_this) {
  ...
  memmove(array_.get() + insert_length, array_.get(), Size() * sizeof(int16_t));
  memcpy(array_.get(), prepend_this.array_.get(), insert_length * sizeof(int16_t));
...
void AudioVector::PushBack(const int16_t* append_this, size_t length) {
  Reserve(Size() + length);
  memcpy(array_.get() + first_free_ix_, append_this, length * sizeof(int16_t));
...
void AudioVector::PopFront(size_t length) {
  if (length >= Size()) {
    ...
    memmove(array_.get(), array_.get() + length, remaining_samples * sizeof(int16_t));
    first_free_ix_ -= length;
  }
}
...
void AudioVector::Extend(size_t extra_length) {
  ...
  memset(array_.get() + first_free_ix_, 0, extra_length * sizeof(int16_t));
...
void AudioVector::InsertAt(const int16_t* insert_this,
  ...
  int16_t* insert_position_ptr = array_.get() + position;
...
void AudioVector::OverwriteAt(const int16_t* insert_this,
  ...
  memcpy(array_.get() + position, insert_this, length * sizeof(int16_t));
...
void AudioVector::CrossFade(const AudioVector& append_this,
  ...
  for (size_t i = 0; i < fade_length; ++i) {
    alpha -= alpha_step;
    *(array_.get() + position + i) = (alpha * (*(array_.get() + position + i)) +
