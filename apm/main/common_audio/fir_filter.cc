...
FIRFilterC::FIRFilterC(const float* coefficients, size_t coefficients_length)
  ...
  for (size_t i = 0; i < coefficients_length_; ++i) {
    *(coefficients_.get() + i) = coefficients[coefficients_length_ - i - 1];
  }
  memset(state_.get(), 0, state_length_ * sizeof(*state_.get()));
}
...
void FIRFilterC::Filter(const float* in, size_t length, float* out) {
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    for (j = 0; state_length_ > i && j < state_length_ - i; ++j) {
      out[i] += (*(state_.get() + i + j)) * (*(coefficients_.get() + j));
    }
    for (; j < coefficients_length_; ++j) {
      out[i] += in[j + i - state_length_] * (*(coefficients_.get() + j));
    }
  }
  ...
  if (length >= state_length_) {
    ...
    memmove(state_.get(),
            state_.get() + length,
            (state_length_ - length) * sizeof(*state_.get()));
    memcpy(state_.get() + state_length_ - length, in, length * sizeof(*in));
  }
}
