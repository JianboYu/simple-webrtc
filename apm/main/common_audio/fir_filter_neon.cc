...
FIRFilterNEON::FIRFilterNEON(const float* coefficients,
  ...
  memset(coefficients_.get(), 0.f, padding * sizeof(*coefficients_.get()));
  ...
  for (size_t i = 0; i < coefficients_length; ++i) {
    *(coefficients_.get() + i + padding) = *(coefficients_.get() + coefficients_length - i - 1);
  }
  memset(state_.get(),
         0.f,
         (max_input_length + state_length_) * sizeof(*state_.get()));
}

void FIRFilterNEON::Filter(const float* in, size_t length, float* out) {
  assert(length > 0);

  memcpy(state_.get() + state_length_, in, length * sizeof(*in));

  // Convolves the input signal |in| with the filter kernel |coefficients_|
  // taking into account the previous state.
  for (size_t i = 0; i < length; ++i) {
    float* in_ptr = state_.get() + i;
  ...
  memmove(state_.get(), state_.get() + length, state_length_ * sizeof(*state_.get()));
}
