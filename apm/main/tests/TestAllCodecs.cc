...
#include "main/interface/audio_coding_module.h"
#include "main/interface/audio_coding_module_typedefs.h"
...
void TestAllCodecs::RegisterCodec(const char* codec_name, int32_t sampling_freq_hz,
                         int rate, int packet_size, int extra_byte, const char* out_name) {
  const std::string file_name = std::string("test.pcm");
  infile_a_.Open(file_name, sampling_freq_hz, "rb");
  printf("Starting coding %s freq: %d rate: %d\n", codec_name, sampling_freq_hz, rate);
  ...
  outfile_b_.Open(out_name, sampling_freq_hz, "wb");
  Run(channel_a_to_b_);
...
void TestAllCodecs::Perform() {
  ...
  acm_a_->InitializeSender();
  acm_b_->InitializeReceiver();

  RegisterCodec("PCMA", 8000, 64000, 160, 0, "test_out_pcma_8k.pcm");
  RegisterCodec("PCMU", 8000, 64000, 160, 0, "test_out_pcmu_8k.pcm");
  RegisterCodec("ILBC", 8000, 13300, 240, 0, "test_out_ilbc_8k.pcm");
  RegisterCodec("L16",  16000, 256000, 640, 0, "test_out_l16_16k.pcm");
  RegisterCodec("G722", 16000, 64000, 160, 0, "test_out_g722_16k.pcm");
  RegisterCodec("OPUS", 48000, 32000, 480*4, -1, "test_out_opus_48k.pcm");
  RegisterCodec("OPUS", 48000, 6000, 480, -1, "test_out_opus_48k_6k.pcm");
  return;

#define OPUS
#ifdef L16
  const char *codec_name = "L16";
  CodecInst my_codec_param;
  int32_t sampling_freq_hz = 16000;
  int32_t chn = 1;
  int32_t rate = 256000;
  int32_t packet_size = 640;
#endif

#ifdef G722
  const char *codec_name = "G722";
  CodecInst my_codec_param;
  int32_t sampling_freq_hz = 16000;
  int32_t chn = 1;
  int32_t rate = 64000;
  int32_t packet_size = 576;
#endif
#ifdef OPUS
  const char *codec_name = "OPUS";
  CodecInst my_codec_param;
  int32_t sampling_freq_hz = 48000;
  int32_t chn = 1;
  int32_t rate = 32000;
  int32_t packet_size = 480*4;
#endif
#ifdef ILBC
...
//add by fanwei------------------start
  ...
  test_count_++;
  OpenOutFile(test_count_);
  char codec_ADPCM[] = "ADPCM";
  RegisterSendCodec('A', codec_ADPCM, 8000, 40000, 256, 0);
#endif
//add by fanwei------------------end
#endif
  ...
  outfile_b_.Close();
#endif
#if 0//(defined(WEBRTC_CODEC_ISAC) || defined(WEBRTC_CODEC_ISACFX))
  if (test_mode_ != 0) {
    printf("===============================================================\n");
  }
  test_count_++;
...
void TestAllCodecs::RegisterSendCodec(char side, char* codec_name,
  ...
  my_codec_param.rate = rate;
  my_codec_param.pacsize = packet_size;
  assert(0 == my_acm->RegisterSendCodec(my_codec_param));
}
  ...
  while (!infile_a_.EndOfFile()) {
    ...
    if (receive_size) {
      if ((static_cast<int>(receive_size) != packet_size_bytes_) &&
          (packet_size_bytes_ > -1)) {
        printf("receive_size: %d packet_size_bytes_: %d\n", receive_size, packet_size_bytes_);
        error_count++;
      }
      ...
      if ((counter > 10) &&
          ...
          (packet_size_samples_ > -1)) {
        printf("timestamp_diff_: %d packet_size_samples_: %d\n", timestamp_diff, packet_size_samples_);
        error_count++;
      }
    }

    // Run received side of ACM.
    assert(0 == acm_b_->PlayoutData10Ms(out_freq_hz, &audio_frame));
...
void TestAllCodecs::OpenOutFile(int test_number) {
  ...
  filename += os.str();
  filename += ".pcm";
  outfile_b_.Open(filename, 48000, "wb");
}
